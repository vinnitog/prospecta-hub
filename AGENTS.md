...
Antes de commit/push, executar `node scripts/check-public-repo.js` e verificar o diff. Dados reais, senhas, hashes reais e arquivos privados nunca entram no Git. Pages publica o CRM estático gerado por `scripts/build-pages.js`, com dados locais no navegador. Não incluir base inicial real ou segredos. A busca autorizada consulta somente o IBGE (municípios) e a API Overpass da Private.coffee com cidade/UF/segmentos; nunca transmitir a carteira, mensagens ou notas. Sem login por decisão explícita do proprietário. O servidor SQLite local deve permanecer restrito a loopback.
